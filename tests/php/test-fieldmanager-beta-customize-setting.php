<?php
class Test_Fieldmanager_Beta_Customize_Setting extends Fieldmanager_Beta_Customize_UnitTestCase {
	protected $field, $context;

	function setUp() {
		parent::setUp();
		$this->field   = new Fieldmanager_TextField( array( 'name' => 'foo' ) );
		$this->context = new Fieldmanager_Beta_Context_Customize( array(), $this->field );
	}

	function test_construction() {
		$actual = new Fieldmanager_Beta_Customize_Setting( $this->manager, $this->field->name, array(
			'context' => $this->context,
		) );

		$this->assertInstanceOf( 'Fieldmanager_Beta_Customize_Setting', $actual );
	...
	/**
	 * Test that properties set by default by Fieldmanager_Beta_Customize_Setting can be overridden.
	 */
	...
	function test_invalid_construction() {
		new Fieldmanager_Beta_Customize_Setting( $this->manager, $this->field->name, array() );
	}
	...
	function test_preview_filter() {
		 ...
		 *
		 * As of 4.7, $_POST is ignored unless the current user can 'customize'.
		 */
		$this->manager->set_post_value( $this->field->name, $preview );
		$this->let_all_users_customize();

		$setting = new Fieldmanager_Beta_Customize_Setting( $this->manager, $this->field->name, array(
			'context' => $this->context,
		) );
